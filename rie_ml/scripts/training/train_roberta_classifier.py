...
class MultiTaskRoBERTaClassifier(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        return {
            'feedback_type_logits': self.feedback_type_head(cls),
            'rule_category_logits': self.rule_category_head(cls),
            'is_actionable_logits': self.is_actionable_head(cls),
            'requires_clarification_logits': self.requires_clarification_head(cls)
        }
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'feedback_type': torch.tensor(
                FEEDBACK_TYPE_LABEL2ID.get(
                    item['feedback_type'] if item['feedback_type'] else 'general_feedback',
                    4
                ),
                dtype=torch.long
            ),
            'rule_category': torch.tensor(
                RULE_CATEGORY_LABEL2ID.get(
                    item['rule_category'] if item['rule_category'] else 'none',
                    6
                ),
                dtype=torch.long
            ),
...
def train_roberta_model():
    ...
    # Paths
    output_dir = Path(__file__).parent.parent.parent / "models" / "roberta_candidate"
                ...
                for line in f:
                    if line.strip():
                        ...
                        if example.get('feedback_type') != 'irrelevant_spam':
                            if example.get('rule_category') is None:
                                example['rule_category'] = 'none'
